...
public class GameUpdater implements Runnable {
	...
	private void updatePhysics() {
			...
			if (ship.canFireWeapon()) {
				...
				bullets.add(
						new Bullet(
								...
								ship.getVelocity().y - Math.cos(direction) * 15, ship.getID()
						)
				);
				ship.setFired();
			}
		...
		if (this.updateCounter % 200 == 0 && asteroids.size() < this.asteroidsLimit && game.isServer()) {
		...
		this.game.getBullets().forEach(bullet -> {
			this.game.getAsteroids().forEach(asteroid -> { // Check collision with any of the asteroids.
				if (asteroid.collides(bullet)) {
					asteroid.destroy();
					bullet.destroy();
					ScoreRenewal(bullet.getMasterShipId());
				}
			});
			if (this.game.getSpaceship() != null && this.game.getSpaceship().collides(bullet)) { // Check collision with ship.
				bullet.destroy();
	...
	private void increaseScore() {
		if (this.game.getSpaceship() != null) {
	...
	/**
	 * Renews the score information of the ships
	 *
	 * @param spaceshipID id of the ship
	 */
	private void ScoreRenewal(String spaceshipID) {
		if (this.game.getSpaceship() != null && this.game.getSpaceship().getID().equals(spaceshipID)) {
			increaseScore();
		}

		for (Spaceship ship : this.game.getSpaceships()) {
			if (ship.getID().equals(spaceshipID)) {
				ship.increaseScore();
			}
		}
	}
